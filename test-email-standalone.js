// Standalone email test script (bypasses authentication)
// Run with: node test-email-standalone.js
...
async function testEmail() {
  ...
  console.log('📧 Sending medical invite email...')
  try {
    const inviteResult = await resend.emails.send({
      ...
      `,
      text: 'Test invite email from DizzyDashboard'
    })
    ...
    console.error('❌ Failed to send medical invite email:', error.message)
  }
  ...
  try {
    const resetResult = await resend.emails.send({
      ...
      `,
      text: 'Test password reset email from DizzyDashboard. Reset code: TEST1234567890AB'
    })
  ...
  console.log('📥 Check your inbox at benstover@gmail.com')
